...
pub async fn run(args: &Args) -> Result<(Value, u8), local::LocalError> {
    if !args.json || args.config != Path::new("ledger.json") {
        return Ok(error(
            "USAGE",
            "candidate requires --json and no --config",
            2,
        ));
    }
    let r = &args.rest;
    ...
    let result = match r[0].as_str() {
        "init" | "submit" if r.len() == 3 => {
            ...
            if r[0] == "init" {
                pilot::init(path, &raw).await
            } else {
                pilot::execute(path, Some(&raw)).await
            }
        }
        ...
        _ => {
            return Ok(error(
                "USAGE",
                "unknown candidate operation or argument count",
                2,
            ))
        }
    };
...
async fn cancel_submit(path: &Path, raw: &[u8]) -> Result<Value, pilot::CandidateError> {
    let directory = std::env::var_os("LEDGER_ZEN_E2E_PAUSE_DIR")
        .map(PathBuf::from)
        .ok_or(pilot::CandidateError {
            code: "E2E_PROBE",
            exit: 3,
        })?;
    let owned_path = path.to_path_buf();
    let owned_raw = raw.to_vec();
    let caller = tokio::spawn(async move { pilot::execute(&owned_path, Some(&owned_raw)).await });
    ...
    loop {
        ...
        if caller.is_finished() || Instant::now() >= deadline {
            caller.abort();
            let _ = caller.await;
            return Err(pilot::CandidateError {
                code: "E2E_PROBE",
                exit: 3,
            });
        }
    ...
    caller.abort();
    let _ = caller.await;
    std::fs::write(directory.join("cancelled"), b"cancelled").map_err(|_| {
        pilot::CandidateError {
            code: "E2E_PROBE",
            exit: 3,
        }
    })?;
