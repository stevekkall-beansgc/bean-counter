...
fn unavailable(_: impl std::fmt::Debug) -> CandidateError {
    CandidateError {
        code: "UNAVAILABLE",
        exit: 7,
    }
}
fn integrity(_: impl std::fmt::Debug) -> CandidateError {
    CandidateError {
        code: "INTEGRITY",
        exit: 9,
    }
...
fn semantic(e: ledgerlab_core::Error) -> CandidateError {
    CandidateError {
        code: e.code,
        exit: if e.code == "BINDING_CONFLICT" {
            4
        } else if e.code == "UNAUTHORIZED_ORDER" {
            6
        } else {
            3
        },
...
async fn init_owned(path: &Path, setup: Setup) -> Result<Value> {
    ...
    fs::File::open(path.parent().unwrap_or(Path::new(".")))
        ...
        .map_err(unavailable)?;
    let mut store = CandidateStore::open(path, true)
        .await
        .map_err(unavailable)?;
    let result = async {
        store.begin().await.map_err(unavailable)?;
        store
            .initialize(&core::bytes(&setup).map_err(semantic)?)
            .await
            .map_err(unavailable)?;
...
async fn execute_owned(path: &Path, command: Option<Command>) -> Result<Value> {
    ...
    if fs::read(path.join("candidate.marker")).map_err(integrity)? != MARKER {
        return Err(integrity(()));
    }
    let mut store = CandidateStore::open(path, false)
        .await
        .map_err(unavailable)?;
    let result = transact(&mut store, command.as_ref()).await;
    // On errors, explicit rollback where possible, then discard the connection.
    if result.is_err() {
        let _ = store.end(false).await;
    }
    store.close().await;
    #[cfg(feature = "zen-charge-e2e-hooks")]
...
async fn transact(store: &mut CandidateStore, command: Option<&Command>) -> Result<Value> {
    ...
    if core::bytes(&setup).map_err(integrity)? != setup_raw {
        return Err(integrity(()));
    }
    ...
    for (index, (seq, raw, at, saved)) in rows.iter().enumerate() {
        ...
        if *seq != index as i64 + 1
            || time.to_string() != *at
            || core::bytes(&cmd).map_err(integrity)? != *raw
        {
            return Err(integrity(()));
        }
        let decision = state.apply(&cmd, time).map_err(integrity)?;
        if !decision.changed || core::bytes(&decision.response).map_err(integrity)? != *saved {
            return Err(integrity(()));
        }
    }
    ...
    if decision.changed {
        ...
        store
            .append(
                rows.len() as i64 + 1,
                &core::bytes(command).map_err(semantic)?,
                at,
                &core::bytes(&decision.response).map_err(semantic)?,
            )
            .await
            .map_err(unavailable)?;
        #[cfg(feature = "zen-charge-e2e-hooks")]
...
fn crash(point: &str, code: i32) {
    ...
    #[cfg(feature = "zen-charge-e2e-hooks")]
    if std::env::var("LEDGER_ZEN_CANDIDATE_CRASH").ok().as_deref() == Some(point) {
...
fn observed_time() -> Result<u64> {
    ...
    if let Ok(raw) = std::env::var("LEDGER_ZEN_E2E_TIME_US") {
        let invalid = || CandidateError {
            code: "E2E_CLOCK_INPUT",
            exit: 2,
        };
        let at: u64 = raw.parse().map_err(|_| invalid())?;
        if at.to_string() != raw || at > i64::MAX as u64 {
            return Err(invalid());
        }
        return Ok(at);
    }
...
pub fn error(e: CandidateError) -> (Value, u8) {
    (
        json!({"schema":core::PROFILE,"candidate":true,"status":"error","code":e.code}),
        e.exit,
    )
}
