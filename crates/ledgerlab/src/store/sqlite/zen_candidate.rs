...
impl CandidateStore {
    ...
    pub(crate) async fn initialize(&mut self, setup: &[u8]) -> Result<(), ()> {
        ...
        sqlx::query("INSERT INTO candidate_setup VALUES (1, ?)")
            .bind(setup)
            .execute(&mut self.connection)
            .await
            .map_err(|_| ())?;
    ...
    pub(crate) async fn setup(&mut self) -> Result<Vec<u8>, ()> {
        let row = sqlx::query("SELECT body FROM candidate_setup WHERE id=1")
    ...
    pub(crate) async fn rows(&mut self) -> Result<Vec<(i64, Vec<u8>, String, Vec<u8>)>, ()> {
        let rows = sqlx::query(
            "SELECT seq,command,at_us,response FROM candidate_entries ORDER BY seq LIMIT 33",
        )
        ...
        rows.into_iter()
            .map(|r| {
                Ok((
                    r.try_get(0).map_err(|_| ())?,
                    r.try_get(1).map_err(|_| ())?,
                    r.try_get(2).map_err(|_| ())?,
                    r.try_get(3).map_err(|_| ())?,
                ))
            })
            .collect()
    }
    pub(crate) async fn append(
        ...
        sqlx::query("INSERT INTO candidate_entries VALUES (?,?,?,?)")
            .bind(seq)
            .bind(command)
            .bind(at.to_string())
            .bind(response)
