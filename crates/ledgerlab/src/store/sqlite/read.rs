...
pub(super) async fn identity(
    ...
    if row.is_none() {
        let occupied: bool = sqlx::query_scalar("SELECT EXISTS(SELECT 1 FROM outcome_deliveries WHERE tenant=?1 AND environment=?2 AND source=?3 AND external_id=?4) OR EXISTS(SELECT 1 FROM r3_deliveries WHERE tenant=?1 AND environment=?2 AND source=?3 AND external_id=?4)")
