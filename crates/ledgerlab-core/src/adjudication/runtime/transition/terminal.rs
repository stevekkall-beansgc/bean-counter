...
fn round_from_begin(
    ...
    require(
        begin.mode == w::BeginMode::FinishOnly && body.mode == w::RoundBeginMode::FinishOnly,
        "ROUND_MODE",
    )?;
    require(
...
pub(super) fn apply(v: &mut View<'_>, d: &mut Delta, work: &Worksheet) -> Result<usize> {
    ...
    match i.command {
        ...
        w::Command::Begin { payload: p, .. } => {
            ...
            require(
                p.mode == w::BeginMode::FinishOnly && p.preparations.is_empty(),
                "ROUND_MODE",
            )?;
            ...
            let body = w::RoundBegin {
                ...
                mode: w::RoundBeginMode::FinishOnly,
                cutoffs,
            };
        ...
        w::Command::Install { payload: p, .. } => {
            ...
            require(
                p.gateway == *i.host
                    ...
                    && p.outcome == w::InstallOutcome::Committed
                    && gw.installed == selected.predecessor
        ...
        w::Command::AckInstall { payload: p, .. } => {
            ...
            let mut r = state(v, p.round)?;
            require(
                r.status == RoundStatus::Committed && p.proof.host == p.gateway,
                "INSTALL_UNKNOWN",
            )?;
            ...
            require(
                installed.round == p.round
                    ...
                    && installed.outcome == w::InstallOutcome::Committed,
                "INSTALL_BINDING",
            )?;
            let selected = r
                .gateways
