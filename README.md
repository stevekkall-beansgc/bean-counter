...
**Current scope:** one business per private installation with multiple customers; multi-business tenancy and Bean-operated infrastructure are deferred. Operators administer the host, storage and backups. M2 customer and agreement support is qualified for the source-built v0.4.3 schema-8 to v0.5.0 schema-9 local SQLite path only; this does not qualify native packages or other historical stores. The v0.3.0 native packages were tested on macOS 26.6.2/Apple silicon and Ubuntu 24.04.5/x86-64. Resource reservations and completion guarantees, PostgreSQL product support, multi-host writer replacement and advanced comparison are deferred. The amended local phases are complete with owner acceptance; the owner waived separate independent review, which did not occur or pass. Read the [public status](STATUS.md), [supported/deferred matrix](CURRENT-REQUIREMENTS.md), and the [billing product roadmap](docs/billing-roadmap.md).
