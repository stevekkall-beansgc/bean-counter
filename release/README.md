...
The [current status](../STATUS.md), [local SQLite requirements](../CURRENT-REQUIREMENTS.md) and [release profile](local-sqlite.md) govern the published product. [v0.3.0](https://github.com/stevekkall-beansgc/bean-counter/releases/tag/v0.3.0) passed the local-profile release gates and exact-commit compliance CI, and provides an unsigned Apple-silicon macOS and Ubuntu 24.04.5 x86-64 archives with checksums and provenance. The amended local phases are complete with owner acceptance; separate independent review was waived, not passed. Deferred guarantees and platforms have not passed.
...
`targets.toml` freezes the exact design §21/24 matrix. That full-platform matrix has not been certified. The separate local-profile v0.3.0 native packages were tested on macOS 26.6.2 with Apple silicon and Ubuntu 24.04.5 x86-64/glibc 2.39; this does not certify other platforms. Ubuntu 22.04 was an intended Linux build environment in the original plan; inspect finished binaries for actual ABI requirements. No universal Linux/Alpine/kernel promise follows from a Rust target triple.
...
The original Phase 0 [runner reference](https://docs.github.com/en/actions/reference/runners/github-hosted-runners) listed ubuntu-22.04, ubuntu-22.04-arm, ubuntu-24.04, ubuntu-24.04-arm, macos-15 ARM64 and windows-2022. That list was documentation inventory, not execution evidence for the full platform. The local-profile exact-commit compliance CI did run for v0.3.0; the broader native platform matrix remains unverified. Private/larger-runner use requires separate budgeting.
...
At Phase 0, this repository was local and unpublished, with `scripts/check.sh` as its local scaffold gate. That exception ended when the local SQLite profile was published. The current release path uses manifest-owned local QA and exact-commit hosted compliance CI; v0.3.0 also shipped source/build identities, SPDX inventory, checksums and a verified installed walkthrough. The original Phase 7 multi-platform certification remains deferred.
