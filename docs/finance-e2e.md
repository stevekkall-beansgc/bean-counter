...
This walkthrough uses **synthetic** customer and agreement data. It runs on the tested Apple-silicon macOS environment. Download `bean-counter-v0.2.0-aarch64-apple-darwin.tar.gz` and `SHA256SUMS` from the [v0.2.0 release](https://github.com/stevekkall-beansgc/bean-counter/releases/tag/v0.2.0). In the download directory, verify the archive before extracting it:

```sh
python3 - <<'PY'
import hashlib
from pathlib import Path
name = 'bean-counter-v0.2.0-aarch64-apple-darwin.tar.gz'
...
PY
tar -xzf bean-counter-v0.2.0-aarch64-apple-darwin.tar.gz
cd bean-counter-v0.2.0-aarch64-apple-darwin
./ledger billing --help
```
