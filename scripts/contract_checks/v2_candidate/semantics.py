...
def exact_percentage(basis,rate):
    ...
    assert (abs(basis)*abs(n)).bit_length()<=512, 'MATH_TEMPORARY_BOUND'
    product=Fraction(basis)*Fraction(n,d)
    # Core reduces the multiplication before the separate division by 100.
    assert (product.denominator*100).bit_length()<=512, 'MATH_TEMPORARY_BOUND'
    result=product/100
...
def base_receipt(b):
    return dict(schema='ledger-base-receipt/2-candidate.2',target=b['target'],base_evaluation=b['base_evaluation'],target_snapshot=b['target_snapshot'],accepted_at=b['accepted_at'],membership_hash=digest('base-membership',b['members']))
...
def freeze(seed,lookup,deref):
    ...
    assert set(material)=={'event','bundle','context','claim_id','actions','explanations','deltas','consumptions','invocations','received_at','source_authority','costs'}, 'BASE_EVALUATION_FIELDS'
    ...
    projected={k:v for k,v in material['event'].items() if k not in ('binding_id','invocation_id')}
    assert projected==event, 'BASE_EVENT_BYTES'
    assert material['source_authority']['source']==event['source'] and material['source_authority']['active'], 'BASE_AUTHORITY'
    lookup(material['source_authority']['grant'],'evidence')
    ...
    assert ordered([p['binding'] for p in material['bundle']['policies'] if p['binding']['book'] in ('retail','supplier')])==ordered([b['body'] for b in bindings.values()]), 'BASE_BINDING_MATERIAL'
    ...
    for p in postings:
        ...
        assert all(a[k]==b[k] for k in ('amount','binding_id','book','roles')), 'ORIGINAL_BASE_EVALUATION'
    ...
    assert len(limits)==len(tb['limits']) and set(limits)==set(bindings), 'PREMIUM_BOUND_REQUIRED'
    for f in families:
    ...
    assert len(versions)==1, 'FROZEN_POLICY_VERSION'
    for binding_id,row in bindings.items():
        b=row['body']; limit=limits[binding_id]
        same_unit(b['booked_net']);same_unit(limit['premium']);same_unit(limit['discount_capacity'])
        net=sum(int(p['body']['amount']['atoms']) for p in postings if p['body']['binding_id']==binding_id)
        assert net==int(b['booked_net']['atoms'])>=0 and b['booked_net']==limit['discount_capacity'], 'BINDING_DISCOUNT_CAPACITY'
        ...
        if b['book']=='supplier':
            ...
            assert i['maximum_exposure']==b['maximum_exposure'], 'SUPPLIER_EXPOSURE'
