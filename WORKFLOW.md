...
On the tested Apple-silicon macOS environment, download the `v0.2.0` native archive and `SHA256SUMS` from the [Bean Counter release](https://github.com/stevekkall-beansgc/bean-counter/releases/tag/v0.2.0). Verify the archive's SHA-256 against that file, extract it, and run the included `ledger` with the included `scripts/demo-finance.py` and `examples/finance/` in a **new** output directory. The [owner walkthrough](docs/finance-e2e.md) gives exact commands and an optional whole-installation backup check.
