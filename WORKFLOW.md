...
`--checks` adds isolated refusal, formula-text encoding, empty-history, remapping conflict, incremental export, read-revocation and four complete outcome-pricing examples. The CLI integration suite runs that mode. Release QA uses `sh scripts/check-local-billing.sh` and `sh scripts/check-local-billing-e2e.sh`. The amended Phase 6 is complete with owner acceptance and a waiver of separate independent review; no independent technical review PASS is claimed. See [current status](STATUS.md).
